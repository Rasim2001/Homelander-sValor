...
namespace TexturePackerImporter
{
    public class SpritesheetImporter : AssetPostprocessor
    {
        private void OnPreprocessTexture()
        {
            ...
            string importerAssetPath = importer.assetPath;

            if (importerAssetPath.EndsWith("RMSK_n.png"))
            {
                importer.textureType = TextureImporterType.Lightmap;
                importer.mipmapEnabled = false;
            }

            if (importerAssetPath.EndsWith("NRM_n.png"))
                importer.textureType = TextureImporterType.NormalMap;

            if (importerAssetPath.EndsWith("RMSK.png"))
            {
                AssetDatabase.DeleteAsset(importerAssetPath);
            ...
            if (sheet != null)
            {
                Debug.Log("Updating sprite sheet " + importer.assetPath);
#if UNITY_2021_2_OR_NEWER
...
if (!oldIds.Any())
{
    ...
    if (texturesToReimport.Count() == 1)
    {
        ...
        {
            Debug.Log("GetSpriteEditorDataProvider");
            ...
            ISpriteEditorDataProvider dataProvider =
                dataProviderFactories.GetSpriteEditorDataProviderFromObject(importer);
        ...
        {
            Debug.Log("sheetInfoToSpriteRects");

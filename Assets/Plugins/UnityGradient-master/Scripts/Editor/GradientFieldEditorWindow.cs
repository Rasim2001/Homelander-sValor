...
namespace Plugins.UnityGradient_master.Scripts.Editor
{
    public class GradientFieldEditorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.PropertyField(
                selectedProperty.FindPropertyRelative(editMode == EditMode.alpha ? "alpha" : "color"),
                GUILayout.Width(Screen.width - 170));
            ...
            EditorGUILayout.PropertyField(selectedProperty.FindPropertyRelative("time"), new GUIContent("Location"),
                GUILayout.Width(120));
        ...
        bool CheckMouse(Rect gradientArea)
        {
            ...
            if (isDragging && selectedProperty != null)
            {
                ...
                switch (Event.current.type)
                {
                    case EventType.MouseDrag:
                        float targetTime = Mathf.InverseLerp(gradientArea.xMin, gradientArea.xMax,
                        ...
                        isDraggingToRemove = Mathf.Abs(handleCenter - Event.current.mousePosition.y) > 20 &&
                                             array.arraySize > 1;
            ...
            if (GUI.Button(gradientArea, GUIContent.none, GUIStyle.none))
            {
                //Invisible button for color picking
                float targetTime =
                    Mathf.InverseLerp(gradientArea.xMin, gradientArea.xMax, Event.current.mousePosition.x);

                Color pickedColor = currentGradient.Evaluate(targetTime);
        ...
        void DrawKeyLine(Rect bounds, float y, float height, EditMode targetEditMode, SerializedProperty array)
        {
            ...
            for (int i = 0; i < arraySize; i++)
            {
                ...
                {
                    GUI.color = Color.white;
                    GUI.DrawTexture(currentRect,
                        targetEditMode == EditMode.alpha ? alphaHandleHighlight : colorHandleHighlight);
                }
